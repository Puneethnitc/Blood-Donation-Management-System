    ...
    <div>
      ...
      <div style={{ marginTop: 16 }}>
        <Card title="Inventory Summary">
          {((inventory?.summary || []).filter((item) => Number(item.units) > 0)).length === 0 ? (
            <EmptyState text="No inventory summary found." />
          ) : (
            <div className="grid grid-3">
              {inventory.summary
                .filter((item) => Number(item.units) > 0)
                .map((item, i) => (
      ...
      <div style={{ marginTop: 16 }}>
        {issueId ? (
          <Card title="Issued Successfully">
            ...
            {requestId && (
              <p style={{ marginTop: 8 }}><strong>Request ID:</strong> {String(requestId).slice(0, 8)}</p>
            )}
          </Card>
        ) : (
